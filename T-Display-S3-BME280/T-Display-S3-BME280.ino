...
// - Line 53: Set your timezone
// - Line 54: Set your town location
// - Line 55: Set your OpenWeather API key
// - Line 56: Set your units to "metric" or "imperial" here
// - Line 57: Set your WiFi SSID
// - Line 58: Set your WiFi password
...
int zone = 2; // User: Set your timezone here (offset in hours from UTC)
String town = "Town"; // User: Set your town location here
String myAPI = "myAPI"; // User: Set your OpenWeather API key here
const char* units = "metric"; // User: Set your units to "metric" or "imperial" here
const char* wifiSSID = "wifiSSID"; // User: Set your WiFi SSID here
const char* wifiPassword = "wifiPassword"; // User: Set your WiFi password here
...
void printMemoryUsage() {
  Serial.print("Free Heap: "); Serial.print(ESP.getFreeHeap()); Serial.println(" bytes");
    ...
    Serial.print("Total PSRAM: "); Serial.print(ESP.getPsramSize()); Serial.println(" bytes");
...
void getData() {
  if (millis() - lastSensorReadTime >= sensorReadInterval || lastSensorReadTime == 0) {
    ...
    Serial.print("HTTP Response Code (Weather): "); Serial.println(httpResponseCode); // Debug log
    yield();
    ...
    if (httpResponseCode > 0) {
      ...
      Serial.print("Weather JSON Size: "); Serial.println(payload.length()); // Debug log
      yield();
    ...
    Serial.print("HTTP Response Code (Forecast): "); Serial.println(httpResponseCode); // Debug log
    yield();
    ...
    if (httpResponseCode > 0) {
      ...
      DeserializationError error = deserializeJson(doc, payload);
      Serial.print("Forecast JSON Size: "); Serial.println(payload.length()); // Debug log
      yield();
    ...
    Serial.print("Free Heap after getData: "); Serial.println(ESP.getFreeHeap());
  }
}
...
void draw() {
  ...
  sprite.drawString("TEMPERATURE", 15, 10); // Label for temperature section
  sprite.unloadFont();
  ...
  sprite.setTextColor(TFT_WHITE, TFT_BLACK);
  sprite.loadFont(font18);
  sprite.drawString("Humidity", xLabel, yStart + 5); // Humidity label
  sprite.unloadFont();
  sprite.setTextDatum(0);
  sprite.loadFont(midleFont);
  ...
  sprite.setTextColor(TFT_WHITE, TFT_BLACK);
  sprite.loadFont(font18);
  sprite.drawString("Pressure", xLabel, yStart + 5); // Pressure label
  sprite.unloadFont();
  sprite.setTextDatum(0);
  sprite.loadFont(midleFont);
  ...
  sprite.drawString("TOWN:", 6, 107); // Town label
...
void updateData() {
  ...
  Serial.print("Animation offset: "); Serial.println(ani); // Debug log for animation
  ...
  if (millis() - timePased >= sensorReadInterval || timePased == 0) {
    ...
    Serial.print("Current Wmsg: "); Serial.println(Wmsg); // Debug log for current message
    if (Wmsg.isEmpty()) {
...
void setup() {
  ...
  if (!bme.begin(0x76, &Wire)) {
    ...
    while (1); // Halt if BME280 not found
  }
...
void tftTask(void *pvParameters) {
  for (;;) {
    ...
    Serial.print("TFT Task running on Core "); Serial.println(xPortGetCoreID()); // Log task core
  }
}
